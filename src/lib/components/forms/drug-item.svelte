...
<Form.Fieldset {form} name="activeIngredients" class="border border-muted p-3">
	...
	<div class="flex flex-col items-stretch gap-3">
		{#each $formData.activeIngredients as _, i (i)}
			...
			>
				<Form.ElementField {form} name="activeIngredients[{i}].name" class="">
					<Form.Control let:attrs>
						<Form.Label class="">Drug generic name {i + 1}</Form.Label>
						<!-- TODO:fix => input change reset combobox state -->
						<Input bind:value={$formData.activeIngredients[i].name} {...attrs} />
					</Form.Control>
					<FormFieldErrors>
						<Form.FieldErrors />
					</FormFieldErrors>
